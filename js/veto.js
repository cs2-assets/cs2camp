...
export function createVeto(userSide = null) {
  return {
    ...
    mapIds: [],         // per picked map: the imported CS result's matchUuid, or null until imported
...
function applySelectionRule(veto, rng = Math.random) {
  ...
  veto.picked = chosen;
  // No ID is generated up front anymore: a played map is matched to one of the
  // plugin's uploaded results at import time, and that result's matchUuid is
  // stored here then (see import-result in app.js).
  veto.mapIds = chosen.map(() => null);
